...
Third-person mech shooter. It plays like a shooter wearing MechWarrior's clothes: you aim with a reticle and there is no lock-on, while the machines still move and animate like mechs. The engine is Godot 4 (GL Compatibility renderer), shipped to the web first.

## Decisions so far

| Area | Decision |
| --- | --- |
| Camera | Third person, over the shoulder. No cockpit view. |
...
| Movement | Agile: 65 km/h run, 50 km/h strafe. Shift + A/D is a sideways dodge with two recharging charges. |
| Threats | Enemy rounds (sabots) are slow, glowing and telegraphed so players can see them coming, dodge and return fire. |
...
Goal: one polished tutorial that proves the core loop is fun: move, see the incoming round, dodge, return fire, direct the lance.

- [x] Start screen with the Iron Wake story and roster
- [x] Dropship intro film, then the in-engine drop and landing
- [x] Knife Cell Striker: agile locomotion, procedural walk, strafe and dodge animation
...
- [x] Tutorial: Operation Dry Wash (4 waypoints, drone kill, dodge drill, tac-map drill, drone wave, extraction)
...
- Playtest the tuning numbers: run speed, dodge distance and cooldown, sabot speed, drone fire rate
...
- Gamepad support (twin stick; dodge on a shoulder button)
- Settings: mouse sensitivity, invert Y, volume sliders
- Loading screen art

## Phase 1: combat depth
- First enemy mech (Sable "Lancer") with a sabot cannon and its own dodges
